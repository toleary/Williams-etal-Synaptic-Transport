...
In order to maintain appropriate connectivity, or to alter it in response to external events, numerous enzymes and macromolecules must be repeatedly replenished and distributed amongst a staggering number of synaptic contacts.
A human might draw inspiration from FedEx to design a solution based on transport hubs and an addressing system.
However, it is unclear how such a sophisticated scheme could be implemented on a microscopic scale in a flexible and robust manner.
Futhermore, other biological transport systems (e.g. in ant colonies; \cite{Deneubourg_1990}) utilize more decentralized strategies based on simple local cues and decisions.
Is it possible to achieve reliable transport in dendrites without an addressing system?
What are the limitations and potential advantages of a decentralized transport system, and how might these constrain the behavior of synaptic plasticity `rules' and, by extension, circuit connectivity?
