...
In order to maintain appropriate connectivity, or to alter it in response to external events, numerous enzymes and macromolecules must be repeatedly replenished and redistributed amongst a staggering number of synaptic contacts.
Many human-designed transport and communication networks solve analogous problems by enumerating all possible destinations in an addressing system.
A coarse addressing system might be implemented by localization sequences in the 3$'$-untranslated regions of mRNAs -- which, for example, control whether mRNA is sequestered in the nucleus or into dendrites \citep{Andreassi2009}.
However, it is unclear whether a fine-scale dendritic addressing system is feasible or even desirable given the diversity of neural morphologies, the stochastic formation of synaptic contacts, and the changing demands of the system.
...
Such self-organizing systems often enjoy a number of advantageous properties -- including scalability, flexibility, and fault-tolerance -- and are therefore a topic of substantial current interest to roboticists and engineers \citep{Werfel2014,Rubenstein2014}.
We asked how a decentralized transport system for synaptic cargo might be organized, ?
How would a decentralized transport system be expected to behave, and how might constrain the behavior of synaptic plasticity `rules' and, by extension, circuit connectivity?
...
The mechanisms that detach and sequester cargo at specific places in dendrites are not fully understood, but are known to depend on local biochemical signals that can be influenced by synaptic activity, such as transient elevations in cyclic AMP or $[Ca^{2+}]$ \citep{Mironov_2007,Wang_2009}.
...
Let $x_n$ denote the position of a particle along a 1-dimensional cable at timestep $n$. Let $v_n$ denote the velocity of the particle at timestep $n$; for simplicity, we assume the velocity can take on three discrete values, $v_n = \{-1,0,1\}$, corresponding to a discrete retrograde movement, pause, or anterograde movement. As a result, $x_n$ is constrained to take on integer values. In the simplest model, we assume that $v_n$ is drawn with fixed probabilities $\mathbf{p} = \{p_{-},p_0,p_+\}$. The update rule for position is:
$$x_{n+1} = x_n + v_n$$
...
Another possibility is that the velocity is updated in a history-dependent manner. For example, if the motor was moving in an anterograde direction on the previous timestep, then it may be more likely to continue to moving in that direction in the next time step. To incorporate this possibility, we change the velocity update rule to follow \textit{conditional} probabilities:
$$v_{n+1} = \left\{\begin{matrix}
-1 & \text{with probability } p(-|v_n)\\ 
0 & \text{with probability } p(0|v_n)\\  
1 & \text{with probability } p(+|v_n)
\end{matrix}\right.$$
We can summarize all of these conditional probabilities in a table:
\begin{center}
  \begin{tabular}{ l | c  c  c }
    & $v_n=-1$ & $v_n=0$ & $v_n=1$ \\
    \hline
    $v_{n+1} = -1$ & $p(-|-)$ & $p(-|0)$ & $p(-|+)$ \\ 
    $v_{n+1} = 0$  & $p(0|-)$ & $p(0|0)$ & $p(0|+)$ \\ 
    $v_{n+1} = 1$  & $p(+|-)$ & $p(+|0)$ & $p(+|+)$ \\
    \hline
  \end{tabular}
\end{center}
The extreme case of this model occurs when the velocity never changes: 
\begin{center}
  \begin{tabular}{ l | c  c  c }
    & $v_n=-1$ & $v_n=0$ & $v_n=1$ \\
    \hline
  ...
  \end{tabular}
\end{center}
We introduce a parameter $k \in [0,1]$ to linearly interpolate between this extreme case and the first model (i.e. with each column given by $\mathbf{p} = \{0.20,0.35,0.45\}$). The rest of the panels in the figure show individual runs, 5000 molecule simulations, and mass action approximations for $k=0.25$ and $k=0.5$. All particles start at $x_0 = 0$ with $v_{-1}$ chosen randomly.
...
The desired steady-state distribution, $\mathbf{\tilde{u}}$, is an eigenvector that spans the nullspace of $A$. It is simple to show that all other eigenvalues $A$ are negative using the Gershgorin circle theorem; thus, the fixed point described by equations 2 and 4 is stable. The convergence rate is determined by the non-zero eigenvalue with smallest magnitude of $A$. We can increase the convergence rate (while still achieving $\mathbf{\tilde{u}}$) by multiplying all rate constants by some common value. However, this trivial strategy ignores the fact that there are biophysical limits on how fast molecular motors can move.
...
This work was supported by Howard Hughes Medical Institute (T.S.), NIH Grant 1P01NS079419 (E.M and T.O.), NIH Grant P41GM103712 (A.H.W., C.O. and T.S.), Charles A. King Trust (T.O.), and the Deptartment of Energy Computational Science Graduate Fellowship (A.H.W.).
