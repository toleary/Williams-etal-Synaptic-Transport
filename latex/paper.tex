...
We have thusfar considered models that produce a desired distribution of cargo to arbitrary precision. However, real neurons might tolerate some sloppiness in synaptic trafficking (or even utilize it as a substrate for complex plasticity rules). Thus, we examined the consequences of relaxing the separation of time scales between transport and detachment. In general, this improves the speed at which cargoes are delivered to synapses, but decreases the accuracy or specificity of delivery.

To illustrate this, we considered an experiment in which cargo is delivered to recently activated synaptic sites.
